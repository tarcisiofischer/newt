...
class WaterParticleArray(ParticleArray):
    def __init__(self, number_of_particles):
        ParticleArray.__init__(
            self, mass=0.001, number_of_particles=number_of_particles
        )
        self.reset()
    ...
    def reset(self):
        ...
        self.velocity[:] = np.random.rand(self.number_of_particles * 3).reshape(
        ...
        self.position[:] = np.tile([-0.5, -2.0, 0.0], self.number_of_particles).reshape(
            self.number_of_particles, 3
        )
    ...
    def update(self, delta_t):
        ...
        self.velocity[positions] = np.random.rand(number_of_deaths * 3).reshape(
            number_of_deaths, 3
        )
        ...
        self.position[positions] = np.tile([-0.5, -2.0, 0.0], number_of_deaths).reshape(
            number_of_deaths, 3
        )
